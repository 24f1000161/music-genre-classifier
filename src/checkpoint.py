...
def _local_override_path() -> Path | None:
    raw = os.getenv("LOCAL_CHECKPOINT_PATH", "").strip()
    if not raw:
        return None
    p = Path(raw)
    if not p.exists():
        raise FileNotFoundError(
            f"LOCAL_CHECKPOINT_PATH points to a missing file: {p}"
        )
    return p
...
def ensure_checkpoint(checkpoints_dir: Path, output_dir: Path) -> Path:
    ...
    direct_checkpoint = output_dir / "resnet50_1hour_best.pth"
    if direct_checkpoint.exists():
        return direct_checkpoint

    raise FileNotFoundError(
        "No local checkpoint found. Expected checkpoints/manifest.json with chunk files, "
        "or models/resnet50_1hour_best.pth, or LOCAL_CHECKPOINT_PATH."
    )
